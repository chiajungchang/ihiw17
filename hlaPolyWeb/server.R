...
function(input, output) {
	...
	hlaData <- eventReactive(input$versionInput,{
		hlaDataInit(input$versionInput)
	})
	genAllele<-eventReactive(input$nucAllele,{
		locus = sub("\\*.*","",input$nucAllele)
		allele2d = sub(":.*","",input$nucAllele)
		if(is.na(hlaData()$refAllele[allele2d])){
			genAllele_local = as.character(hlaData()$refAllele[locus])
		}
		else{
			genAllele_local = as.character(hlaData()$refAllele[allele2d])
		}
		genAllele_local
